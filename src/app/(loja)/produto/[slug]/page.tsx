...
export default function ProdutoPage() {
  ...
  useEffect(() => {
    fetch(`/api/products?slug=${slug}`).then(r => r.json()).then(data => {
      if (data && !data.error) {
        setProduct(data);
  ...
  const MainImage = (
    <div className="mockup-bg" style={{ position: "relative", overflow: "hidden", aspectRatio: "1 / 1" }}>
        ...
        <section className="px-8 pt-4 pb-20">
          <div style={{ maxWidth: 1440, margin: "0 auto", display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: "80px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "72px 1fr", gap: 12 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {product.images.map((src, i) => (
                  <button key={i} onClick={() => setCurrentImageIdx(i)} className="mockup-bg"
                    style={{ aspectRatio: "1", overflow: "hidden", border: i === currentImageIdx ? "1px solid var(--ink)" : "1px solid var(--line-soft)", padding: 0, cursor: "pointer" }}>
              ...
              {/* Imagem principal */}
              <div className="mockup-bg" style={{ position: "relative", overflow: "hidden", aspectRatio: "4 / 5" }}>
                {hasCustomization && (logoPreview || logoFile) ? (
